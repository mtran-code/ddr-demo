class DDRCurveFitting {
  ...
  fitCurve() {
    ...
    if (this.fitType === "biphasic") {
      ...
      const objFn = (params) =>
        this.objectiveFunction(params, sortedPoints, "biphasic", useHuber);
      this.fittedCurve = {
        type: "biphasic",
      ...
      const objFn = (params) =>
        this.objectiveFunction(params, sortedPoints, "monophasic", useHuber);
      this.fittedCurve = {
        type: "monophasic",
        params: this.nelderMead(objFn, initialParams),
      };
  ...
  calculateMetrics(sortedPoints) {
    if (
      !this.fittedCurve ||
      sortedPoints.length < this.advancedConfig.minPointsForFit
    ) {
    ...
    const meanViability =
      sortedPoints.reduce((sum, p) => sum + p.viability, 0) /
      sortedPoints.length;

    for (const point of sortedPoints) {
      const predicted =
        this.fittedCurve.type === "biphasic"
          ? this.biphasicFunction(
              point.concentration,
              this.fittedCurve.params
            ) * 100
          : this.hillFunction(point.concentration, this.fittedCurve.params) *
            100;
  ...
  updateStats() {
    ...
    if (
      this.metrics.ic50 !== null &&
      !isNaN(this.metrics.ic50) &&
      isFinite(this.metrics.ic50)
    ) {
    ...
    if (this.dataPoints.length < this.advancedConfig.minPointsForFit) {
      statusElement.textContent = `Need ${
        this.advancedConfig.minPointsForFit - this.dataPoints.length
      } more points`;
  ...
  drawAxes() {
    ...
    this.ctx.lineTo(
      this.canvas.width - rightPadding,
      this.canvas.height - topPadding
    );
    this.ctx.stroke();
    ...
    for (let logConc = -3; logConc <= 2; logConc++) {
      ...
      this.ctx.fillText(
        `10^${logConc}`,
        x,
        this.canvas.height - topPadding + 20
      );
    }
  ...
  drawFittedCurve() {
    ...
    for (let i = 0; i <= numPoints; i++) {
      const x =
        padding +
        (i / numPoints) * (this.canvas.width - padding - rightPadding);
  ...
  exportCsv() {
    ...
    const sortedPoints = this.getSortedDataPoints();
    for (const point of sortedPoints) {
      csv += `${point.concentration.toFixed(5)},${point.viability.toFixed(
        5
      )}\n`;
    }
    ...
    csv += "\nMetrics\n";
    csv += `R-squared,${
      this.metrics.rSquared !== null ? this.metrics.rSquared.toFixed(4) : "N/A"
    }\n`;
    csv += `IC50,${
      this.metrics.ic50 !== null && !isNaN(this.metrics.ic50)
        ? this.metrics.ic50.toExponential(2)
        : "N/A"
    }\n`;
    csv += `AUC,${
      this.metrics.auc !== null ? this.metrics.auc.toFixed(3) : "N/A"
    }\n`;
